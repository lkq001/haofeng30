...
@section('content')
    ...
    <div class="Hui-article">
		...
		<span class="l">
		<span class="btn btn-primary radius" data-toggle="modal" data-target="#addModel"><i class="Hui-iconfont">&#xe600;</i> 添加仓库</span>
		</span>
            ...
            <div class="mt-20">
                <table class="table table-border table-bordered table-hover table-bg table-sort">
                    ...
                    <tbody>
                    @if($warehouseLists[0])
                        @foreach($warehouseLists[0] as $k => $v)
                            <tr class="text-c">
                                <td>{{ $v->id }}</td>
                                <td class="text-l">{{ $v->name }}</td>
                                <td id="orderBy" data-id="{{ $v->id }}"
                                    data-url="{{ route('admin.warehouse.order') }}"
                                ...
                                <td class="f-14">
                                    <button id="editShowModel" class="btn btn-success size-S radius"
                                            data-id="{{ $v->id }}"
                                            data-url="{{ route('admin.warehouse.edit') }}">编辑
                                    </button>
                                    @if($v->status == 1)
                                        <button id="changeStatus" class="btn btn-warning size-S radius"
                                                data-id="{{ $v->id }}"
                                                data-url="{{ route('admin.warehouse.status') }}"
                                    ...
                                    <button id="destroy" class="btn btn-danger size-S radius" data-id="{{ $v->id }}"
                                            data-url="{{ route('admin.warehouse.destroy') }}">删除
                                    </button>
                            </tr>
                            @if(isset($warehouseLists[$v->id]))
                                @foreach($warehouseLists[$v->id] as $key => $val)
                                    <tr class="text-c">
                                        <td>{{ $val->id }}</td>
...
@section('javascript')
    ...
    <script type="text/javascript" src="{{ asset('/admin/js/warehouse.js') }}"></script>
