...
@section('content')
    ...
    <div class="page-container">
		...
		<span class="l">
		<span href="javascript:;" id="destroy-all" data-url="{{ route('admin.card.category.destroys') }}"
        ...
        <div class="mt-20">
                        ...
                        <tr class="text-c">
                            ...
                            <td class="f-14">
                                <button id="editShowModel" class="btn btn-success size-S radius" data-id="{{ $v->id }}"
                                        data-url="{{ route('admin.card.category.edit') }}">编辑
                                </button>
                                @if($v->status == 1)
                                    <button id="changeStatus" class="btn btn-warning size-S radius"
                                            data-id="{{ $v->id }}"
                                            data-url="{{ route('admin.card.category.status') }}"
                                ...
                                <button id="destroy" class="btn btn-danger size-S radius" data-id="{{ $v->id }}"
                                        data-url="{{ route('admin.card.category.destroy') }}">删除
                                </button>
                        </tr>
                    @endforeach
                @endif
            ...
            {!! $cardCategoryLists->links() !!}
        </div>
    </div>
@endsection
@section('javascript')

    <script type="text/javascript" src="{{ asset('/admin/js/cardCategory.js') }}"></script>
