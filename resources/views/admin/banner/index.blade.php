    ...
    <div class="Hui-article">
        <div class="page-container">
            ...
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l">
                <a href="{{ route('admin.banner.add') }}" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加幻灯片</a>
                </span>
                    ...
                    @if(collect($bannerLists)->count() > 0)
                        @foreach($bannerLists as $k => $v)
                            <tr class="text-c">
                                ...
                                <td>
                                    {{ $v->getOneBannerCategory->name }}
                                </td>

                                <td id="orderBy" data-id="{{ $v->id }}"
                                    data-url="{{ route('admin.banner.order') }}"
                                ...
                                <td class="f-14">
                                    <button id="editShowModel" class="btn btn-success size-S radius"
                                            data-id="{{ $v->id }}"
                                            data-url="{{ route('admin.banner.edit') }}">编辑
                                    </button>
                                    @if($v->status == 2)
                                        <button id="changeStatus" class="btn btn-secondary size-S radius"
                                                data-id="{{ $v->id }}"
                                                data-url="{{ route('admin.banner.status') }}"
                                    ...
                                    <button id="destroy" class="btn btn-danger size-S radius" data-id="{{ $v->id }}"
                                            data-url="{{ route('admin.banner.destroy') }}">删除
                                    </button>
                            </tr>
                        @endforeach
                    @endif
