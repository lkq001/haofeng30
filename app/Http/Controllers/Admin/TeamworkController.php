...
class TeamworkController extends Controller
{
    ...
    public function index()
    {
        ...
        $count = self::$teamworkStore->count();

        // 获取列表
        $lists = self::$teamworkStore->getAll('', config('config.page_size_l'));
//        dd(collect($lists)->toArray());
    ...
    public function store(Request $request)
    {
        ...
        // 执行添加
        $result = self::$teamworkStore->store($request->except('_token', 'old_price'));
        if ($result) {
    ...
    public function edit(Request $request)
    {
        ...
        $info = self::$teamworkStore->getOneInfo(['id' => $request->id]);
    ...
    public function order(Request $request)
    {
        ...
        // 获取指定数据
        // 判断数据是否存在
        $count = self::$teamworkStore->getOneInfoCount(['id' => $request->id]);
        if (count($count) < 1) {
            return response()->json(['code' => 'SN201', 'message' => '数据不存在!']);
        }
        // 执行修改数据
        $updateStatus = self::$teamworkStore->update($request->id, $request->except('id', '_token'));
        if ($updateStatus) {
    ...
    public function destroy(Request $request)
    {
        ...
        // 执行删除操作
        $destroyStatus = self::$teamworkStore->destroy($request->id);
    ...
    public function status(Request $request)
    {
        ...
        // 判断数据是否存在
        $count = self::$teamworkStore->getOneInfoCount(['id' => $request->id, 'status' => $request->status]);
        if ($count < 1) {
            return response()->json(['code' => 'SN201', 'message' => '数据不存在!']);
        }
        ...
        // 执行修改
        $res = self::$teamworkStore->status($request->id, $status);
    ...
    public function update(Request $request)
    {
        ...
        // 指定添加
        $dataStatus = self::$teamworkStore->update($request->id, $request->except('id', '_token', 'old_price'));
